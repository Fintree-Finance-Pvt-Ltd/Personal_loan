...
export class ExternalApiService {
  ...
  constructor(
    ...
    if (
      !Number.isFinite(this.panApiTimeoutMs) ||
      this.panApiTimeoutMs <= 0
    ) {
  ...
  async verifyPan(input: any) {
    ...
    if (
      ...
      customer.panNumber === normalizedPan
    ) {
      const existingKyc =
    ...
    try {
      ...
      if (!providerResponse) {
        ...
        throw new BadGatewayException({
          success: false,
          message:
            'PAN verification provider returned an invalid response.',
        });
      }

      if (
        providerStatus?.statusCode &&
        providerStatus.statusCode !== 200
      ) {
        await this.markPanFailed(
          customerId,
          requestPayload,
      ...
      if (
        ...
        throw new BadGatewayException({
          success: false,
          message:
            'PAN verification could not be completed.',
        });
      }
      ...
      return {
        success: true,
        message:
          'PAN verified and customer details saved successfully.',
        ...
        data: {
          ...
          customerCode:
            updatedCustomer.customerCode,

          panNumber:
            updatedCustomer.panNumber,

          panVerified:
            updatedCustomer.panVerified,

          onboardingStatus:
            updatedCustomer.onboardingStatus,
  ...
  private async saveVerifiedPan(input: {
    ...
    const dateOfBirth =
      input.normalizedData.dateOfBirth
        ? this.parseIsoDate(
            input.normalizedData.dateOfBirth,
          )
        : null;
    ...
    const gender =
      this.toCustomerGender(
        input.normalizedData.gender,
      );
      ...
      async (transaction) => {
        const updatedCustomer =
          await transaction.customer.update({
            ...
            data: {
              panNumber:
                input.normalizedData.panNumber,
              ...
              fullName:
                input.normalizedData.fullName,

              firstName:
                input.normalizedData.firstName,

              middleName:
                input.normalizedData.middleName,

              lastName:
                input.normalizedData.lastName,
        ...
        const existingKyc =
          await transaction.kycVerificationStatus.findFirst({
            where: {
  ...
  private async markPanInitiated(
    customerId: bigint,
    rawRequest: unknown,
  ): Promise<void> {
    const existingKyc =
  ...
  private async markPanFailed(
    ...
    try {
      ...
      const data = {
        ...
        panApiResponse:
          this.stringifyJson(rawResponse),
      };
  ...
  private normalizePanResponse(
    requestedPan: string,
    response: FinanalyzPanResponse,
  ): NormalizedPanVerificationData {
    ...
    return {
      providerApplicationId:
        providerData?.applicationId || null,
      ...
      maskedAadhaar:
        this.cleanOptionalText(
          panDetails?.maskedAadhaar,
        ),

      aadhaarLastFourDigits:
        this.extractAadhaarLastFourDigits(
          panDetails?.lastFourDigit ||
            panDetails?.maskedAadhaar,
        ),
  ...
  private extractAadhaarLastFourDigits(
    value?: string,
  ): string | null {
    if (!value) {
      return null;
  ...
  private getSafeErrorForStorage(
    error: unknown,
  ) {
    if (
  ...
  private handlePanApiError(
    ...
    if (
      ...
      throw new InternalServerErrorException({
        success: false,
        message:
          'An unexpected error occurred during PAN verification.',
      });
    }
    ...
    this.logger.error(
      `PAN provider request failed. Status: ${
    ...
    if (
      ...
      throw new GatewayTimeoutException({
        success: false,
        message:
          'PAN verification provider did not respond in time.',
      });
    }
    ...
    if (
      ...
      throw new BadRequestException({
        ...
        message:
          providerMessage ||
          'PAN verification request was rejected.',
      });
    }
    ...
    if (
      ...
      throw new BadGatewayException({
        success: false,
        message:
          'PAN verification provider authentication failed.',
      });
    }
    ...
    if (
      ...
      throw new BadGatewayException({
        success: false,
        message:
          'PAN verification provider is temporarily unavailable.',
      });
    }
  ...
  private extractProviderErrorMessage(
    ...
    if (
      ...
      if (
        ...
        const status =
          data.status as Record<
            string,
            unknown
          >;

        if (
          typeof status.statusMessage ===
          'string'
        ) {
